...
contract LikesToken is ReentrancyGuard, ERC20, ERC20Burnable, Pausable, AccessControl, Ownable {
    ...
    function setTokenPrice(uint256 newPrice) external onlyRole(PRICE_UPDATER_ROLE) {
        tokenPrice = newPrice;
    }
    ...
    function addModule(address module) external onlyRole(MODULE_ADMIN_ROLE) {
    require(module != address(0), "Invalid address");
    require(!allowedModules[module], "Module already added");
    allowedModules[module] = true;
    }

    function removeModule(address module) external onlyRole(MODULE_ADMIN_ROLE) {
